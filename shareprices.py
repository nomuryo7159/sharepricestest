...
days = st.sidebar.select_slider('日数',
    options=[
        "1d",
        "5d",
        "1mo",
...
st.write(f"過去 {days}間 の株価") # 取得する日数を表示
...
def get_data(days, tickers):
    ...
    for company in tickers.keys():
        ...
        hist = tkr.history(period=f'{days}') # スライドバーで指定した日数で取得した情報を絞る
...
# 売上高データを取得する関数
def get_revenue_data(tickers):
    revenues = {}
    for company in tickers:
        ticker = yf.Ticker(tickers[company])
        try:
            # 売上高データを取得
            revenue = ticker.financials.loc['Total Revenue'].values[-1] if 'Total Revenue' in ticker.financials.index else "データが見つかりません"
        except KeyError:
            revenue = "データが見つかりません"
...
# 売上高データを取得
revenue_data = get_revenue_data(tickers)

# 取得したデータから抽出するための配列を生成し、companiesに代入
companies = st.multiselect(
    '会社名を選択してください。',
    list(df.index),
    ['TOTO','TOYOTA'], # 最初に表示する企業名を設定
)
...
# 選択した企業の売上高を表示
st.write("選択した企業の最新売上高:")
for company in companies:
    st.write(f"{company}: {revenue_data[company]}")